...
from .voltage_control import MAPDNParallelEnv
...
class ResearchMAPDNParallelEnv(MAPDNParallelEnv):
    ...
    def __init__(self, config, manifest, normalizer=None):
        ...
        self.observation_spaces = {a: Box(-np.inf, np.inf, (self.obs_dim,), np.float32)
        ...
        self.action_spaces = {a: Box(self._action_low, self._action_high, dtype=np.float32)
                              for a in self.possible_agents}
