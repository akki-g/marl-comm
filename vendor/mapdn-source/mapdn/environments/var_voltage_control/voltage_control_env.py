...
class VoltageControl(MultiAgentEnv):
    ...
    def __init__(self, kwargs):
        ...
        # set the data path
        self.data_path = self._resolve_data_path(args.data_path)

        # per-instance RNG
        #
        # the environment must never draw from (or perturb) the process-global
        # numpy stream: under SerialEnv every sibling instance would share one
        # sequence, so a seed would stop isolating a replicate and any other
        # numpy consumer in the process would shift the dynamics
        self._seed = getattr(args, "seed", None)
    ...
    def seed(self, seed=None):
        """re-seed this instance's RNG

        rebuilds the per-instance generator, so seeding one environment never
        touches the stream of any other instance in the process
        """
        self._seed = seed
    ...
    def get_action(self):
        ...
        """
        rand_action = self._rng.uniform(low=self.action_space.low, high=self.action_space.high, size=self.powergrid.sgen["q_mvar"].values.shape)
        return rand_action
    ...
    def _select_start_hour(self):
        ...
        """
        return self._rng.choice(24)
    
    def _select_start_interval(self):
        """select start interval for an episode
        """
        return self._rng.choice( 60 // self.time_delta )
    ...
    def _select_start_day(self):
        ...
        return self._rng.choice(pv_days - episode_days)

    @staticmethod
    def _resolve_data_path(data_path):
        """resolve the data path, falling back to the location of the installed package

        a relative path such as "./mapdn/environments/var_voltage_control/data/case33_3min_final"
        only works when the cwd is the repository root, so if it is not found there
        it is re-anchored on the directory holding the mapdn package
        """
        if os.path.isdir(data_path) or os.path.isabs(data_path):
            return data_path
        package_parent = os.path.abspath(
            os.path.join(os.path.dirname(__file__), os.pardir, os.pardir, os.pardir)
        )
        candidate = os.path.normpath(os.path.join(package_parent, data_path))
    ...
    def _set_demand_and_pv(self, add_noise=True):
        ...
        if add_noise:
            pv += self.pv_std * np.abs(self._rng.standard_normal(pv.shape))
        ...
        if add_noise:
            active_demand += self.active_demand_std * np.abs(self._rng.standard_normal(active_demand.shape))
        ...
        if add_noise:
            reactive_demand += self.reactive_demand_std * np.abs(self._rng.standard_normal(reactive_demand.shape))
    ...
    def _take_action(self, actions):
        ...
        """
        # enforce the declared action range here, at the single point where
        # actions enter the physics: _clip_reactive_power only *scales* by the
        # capacity, so without this an out-of-range action drives the inverter
        # past its physical capability instead of saturating at it
    ...
    def _calc_reward(self, info=None):
        """reward function
        consider 5 possible choices on voltage barrier functions:
            ...
            bump
        """
        # a mutable default would be shared by every instance in the process,
        # so the dict is built per call
        if info is None:
            info = {}
