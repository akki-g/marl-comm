...
Both standalone configs run Identity, Broadcast, Gated, Attention and Graph.
Graph uses full connectivity without self-edges. All actors use the same local
encoder and action head; the learned communication modules use 32-dimensional
messages. PCP trains predators and measures predator returns; its scripted prey
is not part of the communication comparison. MAPDN measures the shared team
return once per environment transition.

The channel and communication implementation supports lower-level numerical
regression tests. The starting configs use the identity channel. Historical
ablation protocols and study-specific controls are available at the revision
listed in [HISTORY.md](HISTORY.md).
