...
from commstudy.tasks.vmas.domain_metrics import pcp_metric_specs
...
def episode(contacts=(0, 2, 1, 0), *, complete=True):
    ...
    fields["sender_visible_receiver_blind_pairs"] = torch.full((steps, 3, 1), 2.)
    fields["sender_visible_receiver_blind_prey_triples"] = torch.full((steps, 3, 1), 2.)
...
def test_no_contact_is_censored_and_not_dropped_from_probability():
    ...
    assert summary["means"]["any_contact"] == .5
...
def test_domain_corruption_fails_instead_of_averaging_it_away(defect):
    ...
    if defect == "missing":
        ...
        rollout[key][0] = {"nan": float("nan"), "negative": -1, "fractional": .5}[defect]
