...
from commstudy.tasks.vmas.domain_metrics import (
    PCP_DOMAIN_PROTOCOL,
...
def test_expected_reward_accounts_for_shaping_and_sharing_without_reward_side_effects(
    ...
    with _env(num_landmarks=0, shape_adversary_rew=shaping,
              adversaries_share_rew=shared) as env:
        scenario = env.scenario
        ...
        for prey_position in ((0.05, 0), (-0.8, -0.8)):
            ...
            actual = torch.stack([scenario.reward(agent)
                                  for agent in scenario.adversaries()], dim=-1).mean(-1)
