...
from commstudy.tasks.torchrl.power_grids import (
    TaskConfig, _make_adapter, make_mapdn_env, PowerGridTaskClass,
)
...
def mapdn_config(tmp_path_factory):
    ...
    source = root / "vendor/mapdn-source/tests/conftest.py"
    spec = importlib.util.spec_from_file_location("mapdn_synthetic_fixture", source)
    ...
    return dataclasses.asdict(TaskConfig(
        data_path=str(data), manifest_path=str(manifest_path), max_steps=4, history=2,
...
@pytest.mark.parametrize("overrides", [
    {"max_steps": 0}, {"history": True}, {"solver_algorithm": "unknown"},
    {"action_scale": 2.0}, {"evaluation_split": "train"}, {"split": "oops"},
    {"split": "validation"}, {"split": "test"},
    {"voltage_weight": -1.0}, {"q_weight": 0.1, "line_weight": 0.1},
    {"state_space": ["vm_pu", "vm_pu"]}, {"unknown_option": 1},
])
...
def test_spec_history_and_terminal_diagnostics(mapdn_config):
    ...
    try:
        ...
        assert torch.equal(obs[:, :obs.shape[-1] // 2], torch.zeros_like(
            obs[:, :obs.shape[-1] // 2]))
...
def test_reset_failure_does_not_retry_or_replace_row(mapdn_config, monkeypatch):
    from mapdn.environments.pettingzoo.research_voltage_control import ResetPowerFlowError
    env = _make_adapter(mapdn_config)
    calls = []
    try:
...
def test_inverter_actions_follow_declared_agent_order(mapdn_config, monkeypatch):
    ...
    try:
        ...
        actions = {a: np.array([fractions[i]], np.float32)
...
def test_runtime_contract_binds_vendor_source_and_normalizer(mapdn_config, monkeypatch, tmp_path):
    ...
    normalizer = TrainingNormalizer.fit(np.array([[1.0], [2.0]]), split="train",
...
def test_training_noise_scales_and_private_rng_survive_sibling_steps(mapdn_config):
    ...
    try:
        ...
        assert np.array_equal(first._env.pv_std,
                              np.asarray(manifest["training_statistics"]["pv_std"]) / 100)
        np.random.random(19)
        for _ in range(3):
...
def test_normalizer_from_another_split_manifest_is_rejected(mapdn_config, tmp_path):
    from commstudy.tasks.torchrl.mapdn_data import TrainingNormalizer
    path = tmp_path / "foreign.json"
    TrainingNormalizer.fit(np.ones((2, 50)), split="train",
                           manifest_sha256="0" * 64).save(path)
...
def test_normalization_calibration_settings_cannot_silently_change(mapdn_config, tmp_path, changed):
    from commstudy.tasks.torchrl.mapdn_data import TrainingNormalizer
    settings = {k: v for k, v in mapdn_config.items()
    ...
    TrainingNormalizer.fit(np.ones((2, 44)), split="train",
                           manifest_sha256=manifest["sha256"]).save(normalization_path)
    with pytest.raises(ValueError, match="settings do not match"):
        _make_adapter({**mapdn_config, **changed, "manifest_path": str(path),
...
def test_managed_experiment_uses_validation_only_for_test_env(mapdn_config, config_root, tmp_path):
    ...
    spec = load_experiment_spec(config_root, ["task=mapdn_voltage_control", "model=comm_identity"])
    spec = dataclasses.replace(spec, task_config={"params": mapdn_config}, experiment={
        **spec.experiment, "max_n_frames": 8, "on_policy_collected_frames_per_batch": 8,
        "on_policy_n_envs_per_worker": 1, "on_policy_minibatch_size": 8,
        "on_policy_n_minibatch_iters": 1, "evaluation": False, "loggers": [],
        "create_json": False, "save_folder": str(tmp_path / "model"),
    })
    ...
    try:
        ...
        assert any(not torch.equal(before, after) for before, after in
                   zip(params_before, experiment.policy.parameters(), strict=True))
    finally:
        experiment.close()
