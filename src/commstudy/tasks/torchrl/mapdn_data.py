...
def _blocks(
    ...
    if boundaries is None:
        if len(fractions) != 3 or any(
            isinstance(f, bool) or not math.isfinite(f) or f <= 0 for f in fractions
...
def build_split_manifest(
    ...
    *,
    ...
    for expected, name in ((expected_n_pv, "pv_active.csv"), (expected_n_loads, "load_active.csv")):
        if expected is not None and _integer(expected, f"{name} expected width") != (
            profiles[name].shape[1]
        ):
...
def _validate_manifest(manifest: Mapping[str, Any]) -> None:
    ...
    for name, (lo, hi) in blocks.items():
        ...
        if not starts or any(
            isinstance(row, bool) or not isinstance(row, int) for row in starts
...
class TrainingNormalizer:
    ...
    def __post_init__(self) -> None:
        ...
        if not np.isfinite(self.mean).all() or not np.isfinite(self.scale).all() or any(
