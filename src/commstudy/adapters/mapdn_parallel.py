...
from commstudy.environments.mapdn.voltage_control.voltage_barrier.voltage_barrier_registry import (
    Voltage_Barrier,
)
from commstudy.environments.mapdn.research import ResearchVoltageControl
from commstudy.environments.mapdn.voltage_control.voltage_control_env import VoltageControl
...
class MAPDNParallelEnv(ParallelEnv):
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self._action_low = np.full((self.action_dim,), self._env.action_space.low, dtype=np.float32)
    ...
    def _validate_config(config: dict[str, Any]) -> None:
        ...
        if barrier not in Voltage_Barrier:
            raise ValueError(
                f"unknown voltage_barrier_type {barrier!r}, expected one of {VALID_BARRIERS}"
            )
        ...
        if not isinstance(episode_limit, int) or episode_limit < 2:
            raise ValueError(f"'episode_limit' must be an int >= 2, got {episode_limit!r}")
    ...
    def step(self, actions: dict[str, np.ndarray]):
        ...
        self._last_diagnostics = {key: float(value) for key, value in info.items()}
    ...
    def render(self):
        raise NotImplementedError(
            "MAPDNParallelEnv does not implement rendering; metadata['render_modes'] is empty."
        )
...
class ResearchMAPDNParallelEnv(MAPDNParallelEnv):
    """Retain the user's adapter API, with strict inputs and corrected dynamics."""

    def __init__(self, config, manifest, normalizer=None):
        # Upstream constructor has a concrete VoltageControl dependency; build
        # the research instance first, then initialize the common wrapper fields
        # without monkeypatching module globals or modifying user-owned code.
        from gymnasium.spaces import Box
        from pettingzoo.utils.env import ParallelEnv
        ...
        self._validate_config(self.config)
        self.render_mode = None
        self._env = ResearchVoltageControl(config, manifest)
        self.num_mapdn_agents = self._env.n_agents
        self.obs_dim = self._env.get_obs_size()
        self.state_dim = self._env.get_state_size()
        self.action_dim = self._env.get_total_actions()
        ...
        self.agents = list(self.possible_agents)
        self.observation_spaces = {
            a: Box(-np.inf, np.inf, (self.obs_dim,), np.float32) for a in self.possible_agents
        }
        self._action_low = np.full((1,), self._env.action_space.low, np.float32)
        self._action_high = np.full((1,), self._env.action_space.high, np.float32)
        self.action_spaces = {
            a: Box(self._action_low, self._action_high, dtype=np.float32)
    ...
    def reset(self, seed=None, options=None):
        if options and "start_row" in options:
            self._env.set_episode_start(options["start_row"])
        return super().reset(seed=seed, options=options)

    def _flatten_actions(self, actions):
        if set(actions) != set(self.possible_agents):
            raise ValueError("Exactly one action for each inverter is required.")
        for value in actions.values():
            a = np.asarray(value)
            if a.shape != (1,) or not np.isfinite(a).all():
                raise ValueError("Each inverter action must have finite shape (1,).")
        return super()._flatten_actions(actions)

    def _observation_dict(self, agents_obs):
        result = super()._observation_dict(agents_obs)
        for agent, value in result.items():
            if not np.isfinite(value).all():
                raise ValueError("Nonfinite MAPDN observation.")
            if self.normalizer is not None:
                result[agent] = self.normalizer.transform(value).astype(np.float32)
        return result
