...
def test_declared_additional_local_input_changes_only_its_identity_receiver():
    ...
    td = TensorDict(
        {"adversary": {"observation": torch.randn(2, 3, 5), "local_sensor": torch.randn(2, 3, 2)}},
        batch_size=[2],
    )
    changed = td.clone()
...
def test_pcp_critic_state_is_identical_across_visibility_conditions(config_root):
    ...
    try:
        ...
        for _ in range(3):
            transitions = [
                env.step(td.update(actions.clone())) for env, td in zip(envs, resets, strict=True)
            ]
            next_td = [transition["next"] for transition in transitions]
            assert torch.equal(next_td[0]["state"], next_td[1]["state"])
            assert torch.equal(next_td[0]["adversary", "reward"], next_td[1]["adversary", "reward"])
            resets = next_td
    finally:
        for env in envs:
            env.close()
...
def test_real_pcp_actor_ignores_privileged_and_other_agent_inputs(config_root, tmp_path):
    spec = example_spec(config_root, experiment={"evaluation": False, "create_json": False})
    spec = dataclasses.replace(
        spec, experiment={**spec.experiment, "save_folder": str(tmp_path / "identity")}
    )
    ...
    try:
        ...
        actor = next(
            module
            for module in experiment.group_policies["adversary"].modules()
        ...
        changed["agent", "observation"] = torch.full_like(changed["agent", "observation"], 1e9)
        ...
        critic = next(
            module
            for module in experiment.losses["adversary"].critic_network.modules()
