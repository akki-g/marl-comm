...
def test_normalization_stops_amplification_compounding_across_rounds(kind):
    ...
    unnormalized = [
        _amplification(kind, rounds=r, weight_gain=100, normalize=False) for r in (1, 2, 3)
    ]
    normalized = [
        _amplification(kind, rounds=r, weight_gain=100, normalize=True) for r in (1, 2, 3)
    ]
...
def test_normalization_adds_exactly_one_scalar_parameter(kind):
    ...
    normalized = sum(p.numel() for p in _seeded(kind, normalize_comm_path=True).parameters())
