...
def test_real_dropout_replay_precedes_update_and_diagnostics_separate_phases(config_root, tmp_path):
    ...
    spec = example_spec(
        ...
        experiment={
            ...
            "evaluation": False,
            "create_json": False,
            "save_folder": str(tmp_path / "benchmarl"),
        },
    )
    ...
    replay = check_behavior_replay(policy, batch, "adversary")
    assert replay["replay_log_prob_max_abs_error"] < 1e-5
    assert replay["replay_transition_count"] == 6000
    batch.set(("adversary", "log_prob"), batch.get(("adversary", "log_prob")) + 0.1)
    ...
    def value(phase, metric):
        values = [
            float(row["value"])
            for row in rows
            if row["group"] == "adversary" and row["phase"] == phase and row["metric"] == metric
        ]
