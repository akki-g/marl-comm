...
def test_explicit_single_component_key_can_read_top_level_context():
    ...
    td = make_tensordict(torch.zeros(BATCH_SIZE, N_AGENTS, N_AGENTS, dtype=torch.bool))
...
def test_sampled_sender_mask_is_written_and_replayed_without_resampling():
    ...
    generated_marker = rollout_output.get((GROUP, "comm_sender_mask_generated")).clone()
...
def test_new_model_configs_build(config_root, config_name, class_suffix):
    ...
    document = example_spec(config_root, method=config_name.removeprefix("comm_")).model_config[
        "groups"
    ]["adversary"]
