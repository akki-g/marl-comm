...
def test_explicit_sender_mask_removes_sender_from_every_receiver():
    ...
    h = torch.tensor([[1.0, 0.0], [0.0, 2.0], [3.0, 4.0]])
    context = CommContext(extras={"sender_mask": torch.tensor([True, False, True])})
...
def test_broadcast_accumulates_detached_scalar_stats_and_resets():
    ...
    assert stats["realized_sender_bits_per_step"] == pytest.approx(3 * MESSAGE_DIM * 32)
    assert stats["realized_bits_per_step"] == pytest.approx(6 * MESSAGE_DIM * 32)
...
def test_max_message_norm_is_a_true_logging_window_maximum():
    ...
    assert module.communication_stats()["max_message_norm"] == pytest.approx(expected_max.item())
